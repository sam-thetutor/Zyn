...
const CreateMarket: React.FC = () => {
  ...
  const { createMarket, isPending, isSuccess, hash, marketCreationFee } = usePredictionMarketCore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isConnected) {
      alert('Please connect your wallet to create prediction markets.');
      return;
    }
    ...
    const marketCreationFeeAmount = marketCreationFee ? parseEther(marketCreationFee) : parseEther('0.01'); // Default to 0.01 CELO
    if (balance && balance.value < marketCreationFeeAmount) {
      alert(`Insufficient balance. You need at least ${formatEther(marketCreationFeeAmount)} CELO to create a market. Current balance: ${formatEther(balance.value)} CELO`);
      return;
    }
  ...
  const isSupportedNetwork = chainId === celo.id || chainId === base.id;
  
  // Temporarily allow all networks to see the form, but show a warning
  if (!isSupportedNetwork) {
    // Show warning but still allow form access
    console.warn('Unsupported network detected:', chainId, 'Expected:', celo.id, 'or', base.id);
  }
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-4xl mx-auto mb-6 p-4 bg-gray-100 rounded-lg">
        ...
        <p className="text-sm text-gray-600">Is Connected: {isConnected ? 'Yes' : 'No'}</p>
        <p className="text-sm text-gray-600">Is Supported Network: {isSupportedNetwork ? 'Yes' : 'No'}</p>
        <p className="text-sm text-gray-600">Address: {address || 'None'}</p>
      </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  You're on Chain ID {chainId}. Please switch to Celo Mainnet (42220) or Base Mainnet (8453) for the best experience.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <div className="inline-flex items-center px-4 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
            ...
            Market Creation Fee: {marketCreationFee || '0.01'} CELO
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full text-black px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-amber-700">
                    Creating a prediction market costs <span className="font-semibold">{marketCreationFee || '0.01'} CELO</span>. 
                  ...
                  {balance && (
                    <p className="text-sm text-amber-700 mt-1">
                      ...
                      {balance.value < (marketCreationFee || parseEther('0.01')) && (
